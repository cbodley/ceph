...
class BoundedKeyCounter {
  /// map type to associate keys with their counter values. because the total
  /// size is known, a flat_map can be used to preallocate all storage on
  /// construction
  using map_type = boost::container::flat_map<Key, Count>;
 ...
 public:
  ...
  Count insert(const Key& key, Count n = 1)
  {
    ...
    if (counters.size() < bound) {
      ...
      if (inserted) {
        // insert may relocate elements, invalidating the sorted pointers
        invalidate_sorted();
      }
    } else {
  ...
  void erase(const Key& key)
  {
    ...
    // erase may relocate elements, invalidating the sorted pointers
    invalidate_sorted();
  ...
  void get_highest(size_t count, Callback cb)
  {
    ...
    if (sorted_count < count) {
      ...
      // sort all entries in descending order up the given position
