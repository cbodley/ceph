...
struct CB_SelfmanagedSnap {
  fu2::unique_function<void(boost::system::error_code, snapid_t) &&> fin;
  CB_SelfmanagedSnap(fu2::unique_function<void(boost::system::error_code,
					      snapid_t) &&> fin)
    : fin(std::move(fin)) {}
  void operator()(boost::system::error_code ec, const ceph::buffer::list& bl) {
    snapid_t snapid = 0;
    if (!ec) {
      try {
	auto p = bl.cbegin();
  ...
  fu2::unique_function<
    void(boost::system::error_code,
	 boost::container::flat_map<
	   std::string, pool_stat_t>) &&> onfinish)
{
...
void Objecter::handle_get_pool_stats_reply(MGetPoolStatsReply *m)
{
  ...
  if (iter != poolstat_ops.end()) {
    ...
    std::move(op->onfinish)({}, std::move(m->pool_stats));
