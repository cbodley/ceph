...
#include "rgw/async/completion.h"
#include <optional>
...
TEST(AsyncCompletion, BindHandler)
{
  ...
  auto b1 = rgw::async::bind_handler(std::move(h1), 5, 'a');
  b1();
  const auto& c1 = b1;
  c1();
  std::move(b1)();
  ...
  auto h2 = [] (move_only&& m) {};
  auto b2 = rgw::async::bind_handler(std::move(h2), move_only{});
  std::move(b2)();

  // references bound with std::ref() and passed to all operator() overloads
  auto h3 = [] (int& c) { c++; };
  int count = 0;
  auto b3 = rgw::async::bind_handler(std::move(h3), std::ref(count));
  EXPECT_EQ(0, count);
  b3();
  EXPECT_EQ(1, count);
  const auto& c3 = b3;
...
TEST(AsyncCompletion, MoveOnly)
{
  ...
  {
    using T = move_only;
    using Completion = rgw::async::Completion<void(error_code), T>;
    auto c = Completion::create(ex1, [&ec1] (error_code ec) { ec1 = ec; }, move_only{});
    Completion::post(std::move(c), boost::asio::error::operation_aborted);
    EXPECT_FALSE(ec1);
  }
  {
    using Completion = rgw::async::Completion<void(error_code)>;
...
TEST(AsyncCompletion, CompletionList)
{
  ...
  using T = rgw::async::AsBase<boost::intrusive::list_base_hook<>>;
  using Completion = rgw::async::Completion<void(), T>;
...
TEST(AsyncCompletion, CompletionPair)
{
  ...
  using Completion = rgw::async::Completion<void(int, std::string), T>;
...
TEST(AsyncCompletion, CompletionReference)
{
  ...
  using Completion = rgw::async::Completion<void(int&)>;
...
TEST(AsyncCompletion, ThrowOnCtor)
{
  ...
  }
  {
    ...
    using Completion = rgw::async::Completion<void(int&), T>;
...
TEST(AsyncCompletion, FreeFunctions)
{
  ...
  auto c1 = rgw::async::create_completion<void(), void>(ex1, [] {});
  rgw::async::post(std::move(c1));

  auto c2 = rgw::async::create_completion<void(int), int>(ex1, [] (int) {}, 5);
  rgw::async::defer(std::move(c2), c2->user_data);
