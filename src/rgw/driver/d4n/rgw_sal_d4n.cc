                        ...
                        RGWGetDataCB* cb, optional_yield y) 
{
  ...
  do {
    ...
    if (source->driver->get_cache_driver()->key_exists(dpp, oid_in_cache, y)) { 
      // Read From Cache
      ...
      if ((part_len != obj_max_req_size) && source->driver->get_cache_driver()->key_exists(dpp, oid_in_cache, y) == 0) { 
        // Read From Cache
        auto completed = source->driver->get_cache_driver()->get_async(dpp, y, aio.get(), oid_in_cache, read_ofs, len_to_read, cost, id); 
