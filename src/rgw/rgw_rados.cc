...
boost::system::error_code RGWRados::init_svc(bool raw)
{
  if (raw) {
    return svc.init_raw(cct, ctxpool->get_io_context(), use_cache);
  }

  return svc.init(cct, ctxpool->get_io_context(), use_cache);
}
               ...
               std::optional<uint64_t>* bytes_transferred)
{
  ...
  RGWRadosPutObj cb(cct, plugin, compressor, &processor, progress_cb, progress_data,
                    [&](const map<string, bufferlist>& obj_attrs) {
                      ...
                      return ceph::from_error_code(processor.prepare());
                    });
               ...
               const DoutPrefixProvider *dpp)
{
  ...
  int ret = ceph::from_error_code(processor.prepare());
  if (ret < 0)
    return ret;
...
int RGWRados::cls_bucket_list_unordered(RGWBucketInfo& bucket_info,
  ...
  *is_truncated = false;
  ...
  if (shard_id >= 0) {
    ...
    if(! MultipartMetaFilter(start.name, key)) {
