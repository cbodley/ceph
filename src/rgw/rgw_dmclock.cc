...
void PriorityQueue::cancel(const ClientId& client_id)
{
  queue.remove_by_client(client_id, false, [] (RequestRef&& request) {
      auto c = static_cast<Completion*>(request.release());
      Completion::dispatch(std::unique_ptr<Completion>{c},
                           boost::asio::error::operation_aborted,
                           dmc::PhaseType::priority);
...
void PriorityQueue::schedule(const dmc::Time& time)
{
  ...
  timer.async_wait([this] (boost::system::error_code ec) {
      ...
      if (ec != boost::asio::error::operation_aborted) {
        process(dmc::get_time());
      }
    });
}

void PriorityQueue::process(const dmc::Time& now)
{
