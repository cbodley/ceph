  ...
  if (!info.user_email.empty()) {
    if (!old_info ||
      ...
      ret = rgw_put_system_obj(store, store->svc.zone->get_zone_params().user_email_pool, info.user_email,
                               link_bl, exclusive, NULL, real_time(), null_yield);
      if (ret < 0)
        return ret;
    }
  }
                                 ...
                                 real_time * const pmtime)
{
  ...
  int ret = rgw_get_system_obj(obj_ctx, pool, key, bl, nullptr,
                               null_yield, &e.mtime);
  if (ret < 0)
    return ret;
                             ...
                             map<string, bufferlist> * const pattrs)
{
  ...
  int ret = rgw_get_system_obj(obj_ctx, store->svc.zone->get_zone_params().user_uid_pool,
                               oid, bl, objv_tracker, null_yield, pmtime, pattrs, cache_info);
  if (ret < 0) {
    return ret;
  }
