...
/// associated boost::asio::io_service. the only real utility of this is to
...
class optional_yield_context {
  boost::asio::io_service *s = nullptr;
  boost::asio::yield_context *y = nullptr;
 public:
  /// construct with a valid yield_context pointer
  explicit optional_yield_context(boost::asio::io_service* s,
  ...
  operator bool() const noexcept { return s && y; }

  /// return a reference to the associated io_service. only valid if non-empty
  boost::asio::io_service& get_io_service() const noexcept { return *s; }
