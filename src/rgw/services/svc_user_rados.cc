...
public:  
  ...
  int remove_old_indexes(const RGWUserInfo& old_info, const RGWUserInfo& new_info, optional_yield y) {
    ...
    if (!old_info.user_id.empty() &&
      ...
      ret = svc.user->remove_buckets_index(ctx, old_info.user_id, y);
      if (ret < 0 && ret != -ENOENT) {
        set_err_msg("ERROR: could not remove buckets index for uid " + old_info.user_id.to_str());
        return ret;
      }
    }
...
int RGWSI_User_RADOS::remove_buckets_index(RGWSI_MetaBackend::Context *_ctx,
                                           const rgw_user& user_id,
                                           optional_yield y)
{
  auto ctx = static_cast<RGWSI_MetaBackend_SObj::Context_SObj *>(_ctx);
  rgw_raw_obj obj = get_buckets_obj(user_id);
                                 ...
                                 optional_yield y)
{
  ...
  ret = remove_buckets_index(_ctx, info.user_id, y);
  if (ret < 0 && ret != -ENOENT) {
    ldout(cct, 0) << "ERROR: could not remove user buckets index object for "
        << info.user_id << ": " << ret << dendl;
    return ret;
  }

  ret = remove_uid_index(_ctx, info, objv_tracker, y);
  if (ret < 0 && ret != -ENOENT) {
    return ret;
  }
