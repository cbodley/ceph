...
public:
  ...
  int init(const string& zonegroup_id, const string& zonegroup_name) {
    std::string_view realm_id; // empty, use default
    ...
    case OPT::ZONEGROUP_LIST:
      {
        ...
        int ret = cfgstore->read_default_zonegroup_id(dpp(), null_yield, realm_id,
                                                      default_zonegroup);
	if (ret < 0 && ret != -ENOENT) {
    ...
    case OPT::ZONE_LIST:
      {
        ...
        int ret = cfgstore->read_default_zone_id(dpp(), null_yield,
                                                 realm_id, default_zone);
	if (ret < 0 && ret != -ENOENT) {
