...
int RGWOwnerStatsCache::init_refresh(const rgw_owner& owner, const rgw_bucket& bucket,
                                     boost::intrusive_ptr<RefCountedWaitObject> waiter)
{
  boost::intrusive_ptr handler = new OwnerAsyncRefreshHandler(
      this, std::move(waiter), owner, bucket);
  ...
  struct visitor {
    ...
    int operator()(const rgw_user& user) {
      std::unique_ptr<rgw::sal::User> ruser = driver->get_user(user);
      return ruser->read_stats_async(dpp, std::move(cb));
    }
    int operator()(const rgw_account_id& accountid) {
      return -ENOTSUP; // TODO: read_account_stats_async()
    }
  };
  int r = std::visit(visitor{dpp, driver, std::move(handler)}, owner);
  if (r < 0) {
                                                 ...
                                                 const DoutPrefixProvider *dpp)
{
  struct visitor {
    ...
    int operator()(const rgw_account_id& acct) {
      return -ENOTSUP; // TODO: call load_account_stats()
    }
  };

  int r = std::visit(visitor{driver, stats, y, dpp}, owner);
  if (r < 0) {
    ldpp_dout(dpp, 0) << "could not read stats for owner " << owner << dendl;
    return r;
  }
