...
class SQLiteRealmWriter : public sal::RealmWriter {
  SQLiteConnectionHandle conn;
  int ver;
  std::string tag;
  std::string realm_id;
  std::string realm_name;
 public:
  SQLiteRealmWriter(SQLiteConnectionHandle conn, int ver, std::string tag,
            ...
            const RGWRealm& info) override
  {
    ...
    if (!::sqlite3_changes(conn->db.get())) {
      conn = SQLiteConnectionHandle{}; // release the connection
             ...
             RGWRealm& info, std::string_view new_name) override
  {
    ...
    if (!conn) {
      return -EINVAL; // can't write after delete
    }
    if (realm_id != info.id || realm_name != info.name) {
      return -EINVAL; // can't modify realm id or name directly
    }
                                        ...
                                        std::unique_ptr<sal::RealmWriter>* writer)
{
  ...
  try {
    ...
    if (writer) {
      *writer = std::make_unique<SQLiteRealmWriter>(
          std::move(conn), row.ver, std::move(row.tag), info.id, info.name);
    }
...
class SQLiteZoneGroupWriter : public sal::ZoneGroupWriter {
 ...
 public:
  SQLiteZoneGroupWriter(SQLiteConnectionHandle conn,
...
class SQLiteZoneWriter : public sal::ZoneWriter {
 ...
 public:
  SQLiteZoneWriter(SQLiteConnectionHandle conn, int ver, std::string tag,
