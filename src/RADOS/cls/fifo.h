...
#ifndef CEPH_RADOS_CLS_FIFIO_H
#define CEPH_RADOS_CLS_FIFIO_H
...
template<>
struct is_error_code_enum<::RADOS::CLS::fifo::errc> {
  static const bool value = true;
};
template<>
struct is_error_condition_enum<::RADOS::CLS::fifo::errc> {
...
// Because coroutines are defective and don't handle multiple returns.
...
class FIFO {
  ...
  static void _read_meta(RADOS* r, const Object& oid, const IOContext& ioc,
    ...
    allocator.construct(m, std::move(handler));
  ...
  auto push_entries(const std::deque<cb::list>& data_bufs,
    ...
    allocator.construct(p, std::move(handler));
...
public:
  ...
  static auto open(RADOS& r, const IOContext& ioc, Object oid, CT&& ct,
    ...
    _read_meta(
      ...
      ca::bind_ea(
	e, a,
	[&r, &ioc, oid, handler = std::move(init.completion_handler)]
	(bs::error_code ec, fifo::info info,
	 std::uint32_t size, std::uint32_t over) mutable {
	  auto f = std::unique_ptr<FIFO>(new FIFO(r, ioc, oid));
	  f->info = info;
  ...
  static auto create(RADOS& r, const IOContext& ioc, Object oid, CT&& ct,
    ...
    r.execute(
      ...
      ca::bind_ea(
	e, a,
	[objv, &r, &ioc, oid, handler = std::move(init.completion_handler)]
	(bs::error_code ec) mutable {
	  ...
	  _read_meta(
	    ...
	    ca::bind_ea(
	      e, a,
	      [&r, &ioc, oid, handler = std::move(handler)]
	      (bs::error_code ec, fifo::info info,
	       std::uint32_t size, std::uint32_t over) mutable {
		auto f = std::unique_ptr<FIFO>(new FIFO(r, ioc, oid));
		f->info = info;
  ...
  class Lister {
	...
	::template rebind_alloc<Lister> a(
      ...
      std::move(h)(ec, list_response{std::move(r), m});
    }
  ...
  public:
    ...
    PartInfoGetter(Handler&& handler)
      : handler(std::move(handler)) {}
  };
  ...
  auto get_part_info(int64_t part_num,
    ...
    allocator.construct(g, std::move(init.completion_handler));
...
private:
  ...
  auto process_journal_entry(const fifo::journal_entry& entry,
    ...
    case fifo::journal_entry::Op::set_head:
      ba::post(ba::get_associated_executor(handler),
